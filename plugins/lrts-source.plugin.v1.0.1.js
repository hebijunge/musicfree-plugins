 ...
 *       通过，2 项🟠契约问题 + 2 项🟡候选风险），对照 MusicFree 官方 plugin.d.ts 修复：
 * ① H1：getRecommendSheetsByTag 条目补规范必填的 id 字段（值=sid，双兼容保留 sid——音流
 *    standalone 插件如 qianqian-v1000.js mapQQSheet 实际以 sid 消费，官方 plugin.d.ts 的
 *    IMusicSheetItem 必填 id，两者并存两端宿主均可达）；
 * ② H2：getMusicSheetInfo 入参兼容歌单项对象（官方签名传 IMusicSheetItem 对象；取
 *    id/sid/url 任一字段再走 parseSheetId，字符串形态照旧兼容 importMusicSheet 口径）；
 * ③ M1：getAlbumInfo / getMusicSheetInfo 返回结构补 albumItem / sheetItem 包裹字段
 *    （官方规范形态），顶层平铺字段保留双兼容（音流宿主消费逻辑待真机确证）；
 * ④ M2：章节 ID 优先取字符串形态 sectionId（getBookMenu 实测 id 为 number，>2^53 奇数
 *    会 JSON 解析精度丢失；sectionId 为 string 保真），数字 id 降为兜底。
 * ⑤（回归测试新发现）：search 对象入参形态 { keyword, type, page } 的 type/page 此前被忽略
 *    （只读位置参数），对象形态搜索恒返回空——补齐两种入参形态兼容，位置参数形态不变。
 *
...
function buildChapterItem(bookId, ch, meta, idx) {
  ...
  // M2（质检 v1.0.1）：优先取字符串形态 sectionId——数字 id 可能 >2^53（奇数会被 JSON 解析
  // 四舍五入精度丢失，实测 id=9432679300010000 恰为偶数，sectionId 为 string 保真），id 降为兜底
  var sectionId = str(ch && (ch.sectionId !== undefined && ch.sectionId !== null && ch.sectionId !== '' ? ch.sectionId : ch.id));
...
async function getAlbumInfoImpl(albumItem, page) {
  ...
  if (parsed.kind === 'book') {
    ...
    var note = meta.sections > chapters.length
      ...
      : '';
    var desc = (meta.desc || '') + note;
    return {
      // M1（质检 v1.0.1）：按官方规范补 albumItem 包裹；顶层平铺字段保留双兼容（音流宿主）
      albumItem: {
        id: parsed.bookId ? 'lrts~book~' + parsed.bookId : aid,
        title: meta.name,
        artist: meta.announcer,
        artwork: meta.cover,
        description: desc,
        worksNum: meta.sections,
        platform: 'lrts'
      },
      title: meta.name,
      artist: meta.announcer,
  ...
  var note2 = meta2.sections > list.length
    ...
    : '';
  var desc2 = (meta2.desc || '') + note2;
  return {
    // M1（质检 v1.0.1）：同上，albumItem 包裹 + 顶层双兼容
    albumItem: {
      id: 'lrts~album~' + parsed.albumId,
      title: meta2.name,
      artist: meta2.announcer,
      artwork: meta2.cover,
      description: desc2,
      worksNum: meta2.sections,
      platform: 'lrts'
    },
    title: meta2.name,
    artist: meta2.announcer,
...
async function getMusicSheetInfoImpl(urlLike) {
  // H2（质检 v1.0.1）：官方签名传歌单项对象（IMusicSheetItem），取 id/sid/url 任一标识；
  // 字符串形态照旧兼容（importMusicSheet 链接导入口径）
  var idStr = (urlLike && typeof urlLike === 'object')
    ? str(urlLike.id || urlLike.sid || urlLike.url)
    : str(urlLike);
  var id = parseSheetId(idStr);
  if (!id) throw new Error('[lrts] 无法识别书籍/专辑链接或 ID: ' + idStr.slice(0, 80));
  var album = await getAlbumInfoImpl({ id: id });
  return {
    // M1（质检 v1.0.1）：sheetItem 包裹（官方规范）+ 顶层平铺双兼容
    sheetItem: {
      id: id,
      title: album.title,
      artist: album.artist,
...
async function getRecommendSheetsByTagImpl(tagItem, page) {
  ...
  for (var i = 0; i < books.length; i++) {
    ...
    items.push({
      source: 'lrts',
      // H1（质检 v1.0.1）：官方 IMusicSheetItem 必填 id；sid 保留双兼容（音流宿主按 sid 消费）
      id: 'lrts~book~' + str(b.id),
...
var plugin = {
  ...
  async search(query, page, type) {
    ...
    // 位置参数 (keyword, page, type)；v1.0.1 修复：对象形态的 type/page 此前被忽略导致空结果
