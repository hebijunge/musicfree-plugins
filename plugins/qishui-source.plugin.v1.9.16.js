/**
 * [v1.9.15 显示名中文化] 顶层新增 name 字段：导入/安装列表显示中文名「汽水音乐」；platform 字段保持英文不变，不影响功能逻辑与既有识别逻辑。
 */
/**
 * 汽水音乐独立源插件（MusicFree）
 * ================================
 * v1.9.16（2026-09-26 第十批调研吸收版，基线 v1.9.15）：
 *  - ① seo_track 歌词第三通道：LYRIC_ADAPTERS.qishui 在「分享页 sentences → bugpk KRC」
 *    之后追加第③段——GET beta-luna.douyin.com/luna/h5/seo_track（免签名免 Cookie），
 *    lyric.type='krc' 走 krcToLrc（与 bugpk KRC 同构）转 LRC，lrc 类型直出；命中 KRC 时
 *    回填 qishuiWordLyricCache（getWordByWordLyric 复用现有 QRC 转换）。接口吸收自
 *    zaoqiu2/douyin-lyric-plugin（2026-09 活跃，仅取接口协议事实，代码自行实现，
 *    上游无 LICENSE 不可整仓照搬）。真网实测：track_id=7512402632118503441 → 200，
 *    KRC 6414 字符（2026-09-26 探针）。
 *  - ② 分享链接形态补齐：SONG_URL_RESOLVERS.qishui 单曲链接正则补 /qishui/song/{id}
 *    形态（上游同款，自有正则原缺）；resolveSongId 短链跟随从仅 qishui.douyin.com/s
 *    扩展到 v.douyin.com 通用短链（跟随 302 后按域名门+既有正则提取，落不到汽水曲目页
 *    如实报「无法识别」，不做无音源的占位条目）。域名门不放宽，防开放重定向误配。
 *  - ③ 抖音热歌榜（hot-content）：CHART_DEFS 新增第 5 榜「抖音热歌榜」，
 *    GET music.douyin.com/api/home/hot-content（免登录），data.hotSongs 的
 *    songId/title/artist/coverUrl 直转条目（实测 13 首，isEnd=true）；getTopListDetail
 *    对该榜走专用分支（chartCache 缓存复用，详情页首曲封面回填列表封面）。上游的
 *    chart_tracks「推荐榜」为种子歌曲相关推荐（随种子漂移非稳定榜单），不吸收。
 *    真网实测：hot-content 200 返回「打火机-Penny」等真实数据（2026-09-26 探针）。
...
var CHART_DEFS = [
  ...
  {
    // v1.9.16 ③：抖音热歌榜（music.douyin.com/api/home/hot-content，免登录，实测 13 首）。
    //   非聚合成员榜（members 为空不走 getAggregatedChart），getTopListDetail 按
    //   special='hot-content' 分支走 fetchHotContentChart；封面由详情页首曲回填。
    id: 'agg-dyhot', title: '抖音热歌榜',
    cover: '',
    special: 'hot-content',
    members: []
  }
...
// ---------- v1.9.16 ③：抖音热歌榜（hot-content，免登录） ----------
// GET https://music.douyin.com/api/home/hot-content（免签名免 Cookie，实测 13 首/次）。
// data.hotSongs[]: { rank, songId, title, artist, coverUrl, favoriteCountText }。
// 条目形态对齐 buildChartItem 输出（id 'c_qishui_'+sid + _src.qishui.trackId），
// 供 getLyric/getMediaSource 直接消费；缓存复用 chartCache（CHART_TTL_MS）。
function fetchHotContentChart() {
  var cached = chartCache['agg-dyhot'];
  if (cached && Date.now() - cached.ts < CHART_TTL_MS) {
    return Promise.resolve(cached.list);
  }
  return axios.get('https://music.douyin.com/api/home/hot-content', {
    timeout: SOURCE_TIMEOUT,
    headers: {
    ...
    var songs = (res.data && res.data.data && res.data.data.hotSongs) || [];
    var list = [];
    for (var i = 0; i < songs.length; i++) {
      var s = songs[i] || {};
      var sid = str(s.songId);
      if (!sid || !str(s.title)) continue;
      list.push({
        id: 'c_qishui_' + sid,
        title: str(s.title),
        artist: str(s.artist) || undefined,
        album: undefined,
        duration: undefined,
        artwork: str(s.coverUrl) || undefined,
        _src: { qishui: { trackId: sid } }
      });
    }
    if (!list.length) throw new Error('抖音热歌榜拉取失败：hotSongs 为空');
    chartCache['agg-dyhot'] = { ts: Date.now(), list: list };
    return list;
  });
}
...
var SONG_URL_RESOLVERS = {
  qishui: function (s) {
    ...
    if (!/^https?:\/\/([a-z0-9-]+\.)*(douyin|qishui)\.com(?:[:\/?#]|$)/i.test(s)) return null;
    // v1.9.16 ②：补 /qishui/song/{id} 形态（上游 douyin-lyric-plugin 同款，
    //   自有正则原缺；/qishui/share/track/{id} 已被 /\/track\/(\d+)/ 覆盖）
    var m = /[?&]track_id=(\d+)/.exec(s) || /\/track\/(\d+)/.exec(s) || /\/qishui\/song\/(\d+)/.exec(s);
    return m ? m[1] : null;
  },
};
...
async function resolveSongId(urlLike) {
  ...
  // v1.9.16 ②：短链跟随从仅 qishui.douyin.com/s 扩展到 v.douyin.com 通用短链
  //   （上游同款思路；302 落地后仍走域名门+正则提取，非汽水曲目页则走原报错分支）
  if (/^https?:\/\/(qishui\.douyin\.com\/s|v\.douyin\.com)/.test(s)) {
    s = await followRedirects(s);
  }
...
// v1.9.16 ①：seo_track 歌词专用取数（不要求 url_player_info——纯歌词场景无取链依赖，
//   取链通道 fetchSeoTrack 的 url 校验不适用）。响应顶层 lyric: { type: 'krc'|'lrc', content }
function fetchSeoTrackLyric(trackId, timeoutMs) {
  var tmo = timeoutMs || CHAN_TIMEOUT.seo_track;
  return axios.get('https://beta-luna.douyin.com/luna/h5/seo_track', {
    params: { track_id: trackId, device_platform: 'web' },
    timeout: tmo,
    ...
    var data = res.data;
    // 兼容包裹形态 { data: { lyric } }
    var ly = data && (data.lyric || (data.data && data.data.lyric));
    if (!ly || !ly.content) throw new Error('qishui seo_track no lyric');
    return ly;
  });
}
...
var LYRIC_ADAPTERS = {
  qishui: async function (raw, item) {
    ...
    } catch (e2) { /* 汽水前两段通道未命中，接力第③段 */ }
    // ③ v1.9.16 seo_track 歌词第三通道（免签名免 Cookie；krc 走 krcToLrc 与 bugpk KRC
    //   同构转换，lrc 类型直出）。命中 KRC 时回填逐字缓存（getWordByWordLyric 复用）。
    try {
      var st = await fetchSeoTrackLyric(raw.trackId);
      var slrc = '';
      if (st.type === 'krc') {
        slrc = krcToLrc(st.content);
        if (slrc) qishuiWordLyricCache[raw.trackId] = String(st.content);
      } else {
        slrc = String(st.content || '');
      }
      if (slrc) return { rawLrc: slrc };
    } catch (e3) { /* 汽水三段通道均未命中 */ }
...
var plugin = {
  ...
  async getTopListDetail(topListItem, page) {
    ...
    // v1.9.16 ③：抖音热歌榜走 hot-content 专用通道（members 为空，不能进聚合）
    var musicList2 = def.special === 'hot-content'
      ? await fetchHotContentChart()
      : await getAggregatedChart(def);
    // v1.2.1：透传 topListItem（含 coverImg），详情页头部可直接渲染封面（baka 对齐）；
    //   v1.9.16 ③：抖音热歌榜无静态封面，详情页用首曲封面回填（列表封面为空属预期）
    if (topListItem && !topListItem.coverImg && musicList2.length && musicList2[0].artwork) {
      topListItem.coverImg = musicList2[0].artwork;
    }
  ...
  _internal: {
    ...
    fetchSeoTrackLyric: fetchSeoTrackLyric, // v1.9.16：歌词第三通道（测试复用，与 fetchSeoTrack 同面）
