 ...
 *    ?msg={曲名 歌手}&br={档位}&n=1（出处：V260925 音源包 HelloWorld音源(中秋快乐).js L346）。
 *    v1.9.15 oiapi 集成版（yinliu-app 仓 e9e7458，2026-09-25）已完成开发与真网实测，因发布断层未入订阅线，
 *    本版移植入订阅基线并修正映射缺陷后统一发布。
 *  - 档位映射修正（2026-09-26 真网实测发现缺陷）：集成版 standard/low→LQ，但 oiapi LQ 实测 64kbps
 *    （晴天 周杰伦 LQ 声明 64kbps 实取 2,158,842B；PQ 128kbps 实取 4,317,311B；HQ 320kbps 实取 10,792,962B），
 *    请求 128k 实取 64k 却上报 128k 属虚标。本版修正：standard/low→PQ（128k）、high→HQ（320k）、
 *    super/hires→SQ（flac），actualQuality 与请求档一致；zq32/atmos/atmos2 不支持直接 reject。
 *  - 五重校验后才放行：① 同曲校验（contentId/copyrightId 精确一致，否则 song+singer
 *    isSameSong——搜索型接口实测乱码关键词会被模糊匹配到无关歌曲，必须拦截）；
 *    ② 请求档位声明在场（types[].level）；③ Range 0-15 魔数守卫（SQ 必须 fLaC，
 *    PQ/HQ 必须 ID3/MPEG sync）；④ 文件总长 vs 声明大小（types[].size，MiB 口径）
 *    比值 [0.85,1.15]，不匹配拒收；⑤ URL 协议/域名白名单（复用既有 isAllowedMediaUrl）。
 *  - 编排：候选链插在官方三通道与海棠之后、302/加密接口之前（channelCandidates +
 *    RESOLVE_ADAPTERS + SEG_TIMEOUT_OVERRIDES 段超时放宽 SOURCE_TIMEOUT，接口+探测
 *    两段网络耗时 2500ms 接力段罩不住）；resolveWithFallback 特例免派生梯；
 *    upgradeMiguQuality super 分支（原生 SQ 派生失败后、跨源酷我兜底前）与 hires
 *    分支（原生 ZQ24/SQ 派生均失败后）接入 oiapi 同源替补，限时 2200ms。
 *  - VIP 歌：SQ 档 VIP 无损受限实测 404（七里香 周杰伦），如实拒收由通道链自然降级，不虚标、不擅自降档请求。
 *  - 现有通道与降级逻辑零删除：官方三通道/h5v2.4/v2.1/302/pc-v2.0/海棠 mg/酷我跨源
 *    兜底全部保留，降级优先级不变（酷我→咪咕→网易云→汽水→酷狗→QQ）。
 * v1.9.15（2026-09-25 接口吸收版，基线 v1.9.14）：kuwo/netease/qq 三源集成调研吸收通道（酷我 kw.php/nxinxz/antiserver-high、网易 eapi 响应解密修复+wy.php 兜底、QQ xunhuisi；详见各源 changelog），本源无代码改动，随包升版；同日 oiapi 集成版（本源 +184 行）因发布断层未入订阅线，本版合并发布。
...
// ==================== [v1.9.15] 第三方取链通道：oiapi（咪咕） ====================
// 出处：V260925 音源包 HelloWorld音源(中秋快乐).js L346（api/quality 映射），
// 2026-09-25 独立实测：LQ/PQ/HQ/SQ 四档均返回 freetyst.nf.migu.cn 真实直链，
// SQ flac 31,529,675 字节（周杰伦 VIP 歌《晴天》Range 206 可播），接口响应 ~0.8s。
// 调用：GET https://oiapi.net/api/MiGu_Music?msg={keyword}&br={quality}&n=1
// 音质映射（请求哪个音质取哪个音质）：standard/low→LQ、high→HQ、super→SQ、
// hires→SQ（接口无 hires 档，任务口径明确 hires 降级取 SQ）；zq32/atmos/atmos2
// 接口不支持，直接 reject 交还候选链，不做任何擅自改档。
// 响应结构（实测）：{code:1, data:{song, singer, singers[], copyrightId, contentId,
//   types:[{level,format,bitrate,size}], url}}。
// 搜索型接口风险（实测：乱码关键词会被模糊匹配到无关歌曲的直链）：必须同曲校验——
// ① contentId/copyrightId 精确一致；② 否则 song+singer 与条目 isSameSong 同曲校验。
// 大小比对（任务门禁：Content-Length/Content-Range 与音质大小比对，不匹配不放行）：
// types[].size 形如 "30.07 MB"，实测为 MiB 口径（30.07 MB ↔ 31,529,675 B，误差 <0.1%）；
// Range 0-15 探测文件总长（Content-Range total / 200 Content-Length），与声明大小
// 比值超出 [0.85, 1.15] 视为档位/内容不符，拒收接力。
// 魔数守卫：SQ 必须 fLaC；PQ/HQ 必须 ID3 / MPEG sync（有损档检出 fLaC 同样拒收）。
var OIAPI_SIZE_RATIO_MIN = 0.85;
var OIAPI_SIZE_RATIO_MAX = 1.15;

function oiapiBrForQuality(quality) {
  if (quality === 'standard' || quality === 'low') return 'PQ';
  if (quality === 'high') return 'HQ';
  if (quality === 'super' || quality === 'hires') return 'SQ';
  return '';
}

function oiapiParseDeclaredBytes(sizeStr) {
  var m = String(sizeStr || '').match(/([\d.]+)\s*(KB|MB|GB)/i);
  if (!m) return 0;
  ...
  return Math.round(v * 1024 * 1024); // MB 口径实测为 MiB
}

// 同曲校验：contentId/copyrightId 精确一致优先；否则 song+singer 走 isSameSong
//（版本互斥 + 时长容差，与 kwFbSearchRid 同曲校验同构）。ladder 内调用 raw 传 null，
// 仅靠 musicItem 标题/歌手比对。
function oiapiSameSong(data, musicItem, raw) {
  if (!data) return false;
  if (raw && raw.contentId && str(data.contentId) === str(raw.contentId)) return true;
  if (raw && raw.copyrightId && str(data.copyrightId) === str(raw.copyrightId)) return true;
  if (!musicItem) return false;
  var singers = (data.singers && data.singers.length) ? data.singers.join('/') : str(data.singer);
  var cand = { title: str(data.song), artist: singers };
  if (!cand.title) return false;
  if (isSameSong(cand, musicItem)) return true;
  // [v1.9.15 修复] oiapi 不返回时长：musicItem 侧带时长时 isSameSong 会落到
  // 「专辑名一致才合并」门槛（cand 无 album 必拒），对真实 migu 搜索条目误拒。
  // 退化为 标题键 + 歌手 + 版本兼容 判定（不比时长、不比专辑）。
  var tA = str(cand.title), tB = str(musicItem.title);
  if (normalizeTitle(tA) !== normalizeTitle(tB) && looseTitleKey(tA) !== looseTitleKey(tB)) return false;
  if (normalizeArtist(cand.artist) !== normalizeArtist(str(musicItem.artist))) return false;
  return versionCompatible(parseVersionTags(tA), parseVersionTags(tB));
}

// Range 0-15 一步探测：魔数校验 + 文件总长提取（206 Content-Range / 200 Content-Length）
function oiapiProbeAndValidate(url, br) {
  return axios.get(url, {
    timeout: RELAY_TIMEOUT,
    headers: { Range: 'bytes=0-15', Referer: 'https://music.migu.cn/' },
    responseType: 'arraybuffer'
  }).then(function (res) {
    if (res.status >= 400) throw new Error('oiapi: probe http ' + res.status);
    var headers = res.headers || {};
    var ctype = String(headers['content-type'] || headers['Content-Type'] || '');
    if (/text\/html/i.test(ctype)) throw new Error('oiapi: probe html');
    var buf = res.data;
    var u8 = buf instanceof Uint8Array ? buf
      : (buf instanceof ArrayBuffer ? new Uint8Array(buf)
        : (buf && buf.buffer instanceof ArrayBuffer
          ? new Uint8Array(buf.buffer, buf.byteOffset || 0, buf.byteLength || buf.length) : null));
    if (!u8 || u8.length < 4) throw new Error('oiapi: probe empty');
    ...
    if (br === 'SQ') {
      if (!isFlac) throw new Error('oiapi: SQ 档魔数非 fLaC');
    } else if (!isId3 && !isMp3) {
      throw new Error('oiapi: ' + br + ' 档魔数非音频');
    }
    var total = 0;
    var cr = headers['content-range'] || headers['Content-Range'];
    var mm = cr && String(cr).match(/\/(\d+)\s*$/);
...
// oiapi 适配器主入口（raw 可空：upgradeMiguQuality 梯内调用时仅靠 musicItem 同曲校验）
function resolveMiguOiapi(raw, quality, musicItem) {
  var br = oiapiBrForQuality(quality);
  if (!br) return Promise.reject(new Error('oiapi: quality ' + quality + ' not supported'));
  var query = (str(musicItem && musicItem.title) + ' ' + str(musicItem && musicItem.artist)).trim();
  if (!query) return Promise.reject(new Error('oiapi: no query'));
  return axios.get('https://oiapi.net/api/MiGu_Music', {
    params: { msg: query, br: br, n: 1 },
    timeout: RELAY_TIMEOUT,
    headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)' }
  }).then(function (res) {
    var data = res.data && res.data.data;
    if (!res.data || res.data.code !== 1 || !data || !data.url) throw new Error('oiapi: no url');
    if (!oiapiSameSong(data, musicItem, raw)) throw new Error('oiapi: 模糊命中非同曲，拒收');
    // 目标档位声明在场校验 + 声明大小解析（请求哪档就要求 types 声明哪档）
    var declared = 0;
    var types = data.types || [];
    for (var i = 0; i < types.length; i++) {
      if (types[i] && types[i].level === br) { declared = oiapiParseDeclaredBytes(types[i].size); break; }
    }
    if (!declared) throw new Error('oiapi: ' + br + ' 档位声明缺失');
    var url = String(data.url);
    return oiapiProbeAndValidate(url, br).then(function (total) {
      if (!total || total < 1024) throw new Error('oiapi: 探测无有效总长');
      var ratio = total / declared;
      if (ratio < OIAPI_SIZE_RATIO_MIN || ratio > OIAPI_SIZE_RATIO_MAX) {
        throw new Error('oiapi: 大小比对不符 total=' + total + ' declared=' + declared);
      }
      var aq = (quality === 'super' || quality === 'hires') ? 'flac'
        : (quality === 'high' ? '320k' : '128k');
      var result = { url: url, actualQuality: aq, quality: aq, channel: 'migu-oiapi' };
      if (total > 0) { result.bytes = total; result.size = total; }
      return result;
    });
  });
}
...
async function upgradeMiguQuality(pqUrl, quality, z3d, musicItem) {
  ...
  if (quality === 'super') {
    ...
    // [v1.9.15] 原生 SQ 派生失败 → oiapi SQ（同源第三方替补，先于跨源酷我兜底）；
    // 单独限时 2200ms，超时/失败继续原降级语义
    try {
      return await withTimeout(resolveMiguOiapi(null, 'super', musicItem), 2200, 'oiapi-sq 超时');
    } catch (eoi) { /* 继续酷我兜底 */ }
  ...
  // [v1.9.15] 原生 ZQ24/SQ 派生均失败 → oiapi SQ（任务口径：hires 降级取 SQ，接口无 hires 档）；
  // 单独限时 2200ms，失败继续降档 HQ
  try {
    return await withTimeout(resolveMiguOiapi(null, 'hires', musicItem), 2200, 'oiapi-hires 超时');
  } catch (eoi2) { /* 降级继续 */ }
...
function channelCandidates(quality) {
  ...
  // [v1.9.15] oiapi 第三方取链通道：官方通道之后、302/加密接口之前
  // （实测 LQ/HQ/SQ 三档真链全通，hires 降级 SQ；作为官方通道全败时的同源替补）
  chain.push('migu~oiapi');
...
var RESOLVE_ADAPTERS = {
  ...
  // [v1.9.15] oiapi 通道含接口请求 + Range 探测两段网络耗时（实测 ~1.5-2s），
  // 2500ms 接力段超时会掐死探测，单独放宽到 SOURCE_TIMEOUT（对齐长青 v1.9.14 先例）
  'migu~oiapi': resolveMiguOiapi,
  'migu~305': resolveMigu305,
  'migu~enc': resolveMiguEnc
};

// [v1.9.15] 接力段超时覆盖：默认 RELAY_TIMEOUT，oiapi 需接口+探测两段预算
var SEG_TIMEOUT_OVERRIDES = { 'migu~oiapi': SOURCE_TIMEOUT };
...
function resolveWithFallback(musicItem, quality) {
  ...
  var attempt = function (idx) {
    ...
    var segTimeout = idx === 0 ? SOURCE_TIMEOUT : (SEG_TIMEOUT_OVERRIDES[source] || RELAY_TIMEOUT);
...
var plugin = {
  ...
  _internal: {
    ...
    resolveMiguOiapi: resolveMiguOiapi,
    oiapiParseDeclaredBytes: oiapiParseDeclaredBytes, // [v1.9.15] 自测用导出
