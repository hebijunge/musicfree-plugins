 ...
 *    参数 web_location/dm_img_list/dm_img_str/dm_cover_img_str/dm_img_inter，对齐浏览器
 *    实测形态——缺这些参数即使登录也返回 subtitles:[]）→ data.subtitle.subtitles[]
 *    选 lan 含 zh/cn 为主歌词、含 en 为 translation（宿主双语展示），两者皆无降级取
 *    第一条 → GET subtitle_url（"//" 前缀补 https）→ body[].from/content 转 LRC。
 *    未配置 Cookie 直接返回 null（AI 字幕匿名拿不到，避免无谓请求）；字幕为空/请求
 *    失败返回 null 交宿主跨源歌词检索（沿 v1.1.0 stub 不 throw 契约，避免中断宿主
 *    lyricManager 整链）。cid 复用 ensureVideoMeta（条目自带 cid 零请求）。
 *    接口吸收自 martin65536/bilibili-musicfree axios 版（2026-09 活跃；仅取接口协议
 *    事实，代码自行实现，上游无 LICENSE 不可整仓照搬；其代理壳版需 Termux 本机跑
 *    Node，形态不适用，未吸收）。
 *  - 已知边界：AI 字幕仅覆盖开启 AI 字幕的视频（不是所有视频都有）；端到端真测需
 *    含 SESSDATA 的真实 Cookie（沙箱无法自备），真机验证待用户配置后进行，匿名分支
 *    与转换逻辑已单测覆盖。
 *  - Cookie 登录态支持：v1.0.1 起既有（userVariables.biliCookie 取链在用），本版仅
 *    在 hint 补 AI 字幕依赖说明，无代码改动。
...
// ==================== 插件导出 ====================
// ==================== 歌词（v1.1.1：AI 字幕降级方案） ====================
// [v1.1.0 P1-9] 原「B 站无官方歌词接口」空 stub；v1.1.1 由 AI 字幕补上（未配置 Cookie
// 或无字幕时仍返回 null 交宿主跨源检索，不 throw——宿主包装层视 throw 为该源整体不可用）。
// B 站字幕链路（martin65536/bilibili-musicfree 实测同款协议，代码自行实现）：
//   1. GET /x/player/wbi/v2?bvid=&cid= 带 Cookie + 风控参数 → data.subtitle.subtitles[]
//      每项 { lan, lan_doc, subtitle_url }；匿名状态下 subtitles 通常为空
//   2. 请求 subtitle_url（"//" 开头补 https）→ { body: [{ from, to, content }] }
//   3. body → LRC（[mm:ss.xx]行文本）
function biliSecondsToLrcTime(sec) {
  if (typeof sec !== 'number' || !isFinite(sec) || sec < 0) sec = 0;
  var m = Math.floor(sec / 60);
  var s = sec - m * 60;
  return (m < 10 ? '0' + m : String(m)) + ':' + (s < 10 ? '0' : '') + s.toFixed(2);
}

function biliSubtitleBodyToLrc(body) {
  if (!Array.isArray(body) || !body.length) return '';
  var lines = [];
  for (var i = 0; i < body.length; i++) {
    var it = body[i] || {};
    var from = typeof it.from === 'number' ? it.from : parseFloat(it.from);
    var content = str(it.content).trim();
    if (!content) continue;
    lines.push('[' + biliSecondsToLrcTime(from) + ']' + content);
  }
  return lines.join('\n');
}

/** 拉单条字幕 JSON（subtitle_url，协议相对补全）→ body 数组 | null */
async function biliFetchSubtitleBody(sub, referer) {
  if (!sub || !str(sub.subtitle_url)) return null;
  var u = str(sub.subtitle_url);
  if (u.indexOf('//') === 0) u = 'https:' + u;
  else if (u.indexOf('http') !== 0) u = 'https://' + u;
  var r = await axios.get(u, {
    timeout: SOURCE_TIMEOUT,
    headers: { 'User-Agent': UA, Referer: referer || 'https://www.bilibili.com/' }
  });
  return Array.isArray(r.data && r.data.body) ? r.data.body : null;
}

async function getLyricImpl(musicItem) {
  // AI 字幕需登录态（SESSDATA），匿名拿不到——直接返回 null，避免无谓请求
  if (!musicItem || !userCookie()) return null;
  try {
    // cid：条目自带（分P/搜索内联）零请求；缺失走 ensureVideoMeta（fetchView 有缓存）
    var cid = musicItem.cid;
    var bvid = str(musicItem.bvid);
    if (!cid) {
      var meta = await ensureVideoMeta(musicItem);
      cid = meta && meta.cid;
      if (!bvid) bvid = (meta && meta.bvid) || '';
    }
    if (!cid || (!bvid && !musicItem.aid)) return null;
    var referer = bvid ? 'https://www.bilibili.com/video/' + bvid : 'https://www.bilibili.com/';
    // 风控参数（web_location/dm_* 族）对齐浏览器实测形态：缺失时即使登录也返回 subtitles:[]
    var r = await axios.get(API_HOST + '/x/player/wbi/v2', {
      params: bvid
        ? {
            bvid: bvid, cid: cid, isGaiaAvoided: false, web_location: 1315873,
            dm_img_list: '[]',
            dm_img_str: 'V2ViR0wgMS4wIChPcGVuR0wgRVMgMi4wIENocm9taXVtKQ',
            dm_cover_img_str: 'QU5HTEUgKE5WSURJQSwgTlZJRElBIEdlRm9yY2UgR1RYIDE2NTAgKDB4MDAwMDFGOTEpIERpcmVjdDNEMTEgdnNfNV8wIHBzXzVfMCwgRDNEMTEpR29vZ2xlIEluYy4gKE5WSURJQ',
            dm_img_inter: '{"ds":[],"wh":[4564,4288,68],"of":[401,802,401]}'
          }
        : { aid: musicItem.aid, cid: cid, isGaiaAvoided: false, web_location: 1315873 },
      timeout: SOURCE_TIMEOUT,
      headers: apiHeaders({ Referer: referer, Origin: 'https://www.bilibili.com', 'Accept-Language': 'zh-CN,zh;q=0.9,en;q=0.8' })
    });
    var body = r.data || {};
    if (body.code !== 0 || !body.data || !body.data.subtitle) return null;
    var subtitles = body.data.subtitle.subtitles || [];
    if (!subtitles.length) return null;
    // 语言选择：zh/cn 为主歌词，en 为 translation（宿主双语展示）；皆无降级第一条
    var zhSub = null, enSub = null;
    for (var i = 0; i < subtitles.length; i++) {
      var lan = str(subtitles[i] && subtitles[i].lan);
      if (!zhSub && /zh|cn/i.test(lan)) zhSub = subtitles[i];
      else if (!enSub && /en/i.test(lan)) enSub = subtitles[i];
    }
    var rawLrc = zhSub ? biliSubtitleBodyToLrc(await biliFetchSubtitleBody(zhSub, referer)) : '';
    var translation = enSub ? biliSubtitleBodyToLrc(await biliFetchSubtitleBody(enSub, referer)) : '';
    if (!rawLrc && !translation) {
      // 降级：任取第一条可用字幕
      for (var j = 0; j < subtitles.length && !rawLrc; j++) {
        rawLrc = biliSubtitleBodyToLrc(await biliFetchSubtitleBody(subtitles[j], referer));
      }
    }
    if (!rawLrc && !translation) return null;
    var out = {};
    if (rawLrc) out.rawLrc = rawLrc;
    if (translation) out.translation = translation;
    return out;
  } catch (e) {
    return null; // 字幕获取失败不中断宿主 lyricManager 跨源链
  }
}
...
var plugin = {
  ...
  userVariables: [
    {
      key: 'biliCookie',
      name: 'B站 Cookie（可选）',
      hint: '浏览器登录 bilibili.com 后，F12 → 网络 → 任一 api.bilibili.com 请求 → 复制完整 Cookie 请求头（需含 SESSDATA、bili_jct）。配置且为大会员账号后可解锁 Hi-Res/Dolby 音质；同时启用 AI 字幕歌词（v1.1.1：B 站视频无官方歌词，AI 字幕为降级方案，仅覆盖开启 AI 字幕的视频）；不配置仅免费档（96k/192k）且无 AI 字幕歌词。注意：音质档位选项在插件加载时评估，配置 Cookie 后需重载插件（重启 App）才会刷新档位选项。',
    },
  ],
  hints: {
    importMusicItem: [
  ...
  getLyric: getLyricImpl, // [v1.1.1] AI 字幕作歌词（无 Cookie/无字幕返回 null 交宿主跨源检索）
};
