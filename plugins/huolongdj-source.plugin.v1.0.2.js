/**
 * [v1.0.2 显示名中文化] 顶层新增 name 字段：导入/安装列表显示中文名「火龙DJ」；platform 字段保持英文不变，不影响功能逻辑与既有识别逻辑。
 */
 ...
 *    buildMusicItem 落 _coverPath（原始封面路径），详情页据此重签 artwork（防盗链 24h 时效）；
 *    ③ extractUid 加边界断言，uid 前不得为字母/数字/连字符/下划线，杜绝 "guid=" 类误匹配。
 * v1.0.1（2026-09-24 交叉质检修复版，质检1号）：① hlReport 用户变量接线——此前仅声明未读取，
 *    现按官方 WebDAV 插件口径经宿主注入的 env.getUserVariables() 读取，off/false/0/no/disabled/关闭
 *    视为关闭上报，未设置默认开；② pure 档码率反推守卫去掉 60s 下限（短曲目同样反推，杜绝虚标 320k），
 *    duration 缺失无法反推时按宁低勿高降标 128k（fLaC 魔数校验通过的不降）。
 ...
 *
 * 接口面（本源能力边界，宁缺毋假）：
 *  - 搜索：GET /api/Music/Search（新式 JSON，小写 MD5 双层签名），Pz=50（实测 Pz=10/20 上游
 *    随机忽略分页直接倾倒全量 ≤1000 条，插件按 (pi-1)*50 切片兜底还原页语义）、sortWay=0、
 *    flag=0，Pi 从 1 起；
...
function buildMusicItem(cfg, raw) {
  ...
  // 原始封面路径（未经防盗链签名）：getMusicInfo 进入详情页时据此重签，规避 24h 时效
  item._coverPath = cover;
  return item;
}
...
// 读取宿主注入的用户变量（对齐官方 WebDAV 插件口径：env.getUserVariables() 返回键值对象）
function getUserVariables() {
  try {
    if (typeof env !== 'undefined' && env && typeof env.getUserVariables === 'function') {
      var uv = env.getUserVariables();
      if (uv && typeof uv === 'object') return uv;
    }
  } catch (e) { /* 宿主未注入或异常：按默认处理 */ }
  return {};
}

// 显式关闭判定：off/false/0/no/disabled/关闭 视为关；未设置/空 = 默认开
function isUserVarOff(v) {
  if (v === undefined || v === null || v === '') return false;
  var s = str(v).trim().toLowerCase();
  return s === 'off' || s === 'false' || s === '0' || s === 'no' || s === 'disabled' || s === '关闭';
}

function hlReportEnabled() {
  return !isUserVarOff(getUserVariables().hlReport);
}

// ==================== 播放统计（CountPlay，未登录可用；best-effort 不阻断取链） ====================

async function reportPlay(cfg, musicid) {
  if (!hlReportEnabled()) return false; // 用户变量 hlReport=off 时跳过上报
  try {
    var envRes = await signedPost('/api/Music/CountPlay', {
      musicid: Number(musicid) || 0,
      duration: 30000,
      device: '2'
    }, cfg);
    return !!(envRes && envRes.retcode === 1);
  } catch (e) {
    return false; // 统计失败静默
  }
}
...
async function getMusicInfoImpl(musicItem) {
  ...
  // 封面防盗链 URL 时效≈24h：条目携带 _coverPath（buildMusicItem 存的原始路径）时重签
  var coverPath = str(musicItem._coverPath);
  if (coverPath) {
    try {
      var cfg = await fetchRuntimeConfig(false);
      out.artwork = cdnSignedUrl(cfg.cdnImg, coverPath, cfg.fdKey);
    } catch (e) { /* 重签失败（fetchRuntimeConfig 内部已回退硬编码）保持原样 */ }
  }
  return out;
}
...
function extractUid(urlLike) {
  ...
  // 边界断言：uid 前不得是字母/数字/连字符/下划线——避免 "guid=12345"、"myuid=1" 误匹配（v1.0.2）
  var m = /(?:^|[^a-zA-Z0-9_-])uid[=:](\d+)/i.exec(text);
  if (m) return m[1];
  // ② 整串就是数字 id
...
plugin._internal = {
  ...
  hlReportEnabled: hlReportEnabled,
  getUserVariables: getUserVariables,
  isUserVarOff: isUserVarOff,
